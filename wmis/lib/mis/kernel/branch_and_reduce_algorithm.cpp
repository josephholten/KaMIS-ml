	...
	set_local_reductions = [this, called_from_fold]() {
		if (this->config.reduction_style == MISConfig::Reduction_Style::DENSE) {
            ...
            if (called_from_fold) {
                ...
                status.reductions = make_reduction_vector<neighborhood_reduction, fold2_reduction, clique_reduction, domination_reduction, twin_reduction, clique_neighborhood_reduction, critical_set_reduction, generalized_fold_reduction, greedy_reduction>(status.n);
            }
        } else if (this->config.reduction_style == MISConfig::Reduction_Style::SIMPLE_ML) {
            if (called_from_fold) {
                status.reductions = make_reduction_vector<ml_reduction>(status.n);
            } else {
                status.reductions = make_reduction_vector<ml_reduction>(status.n);
            }
        } else {
