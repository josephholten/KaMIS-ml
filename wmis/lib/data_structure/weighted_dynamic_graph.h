...
public:
	...
	weighted_dynamic_graph(const std::vector<std::vector<NodeID>>& adj) {
	...
	weighted_dynamic_graph(std::vector<std::vector<NodeID>>&& adj) {
		graph.reserve(adj.size());
